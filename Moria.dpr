...
//done? Make all message lines coloured white.
//done? Fix the display of monster/player status after the stats changes
//		in dungeon.  Need to fix maximum weight display after strength
//		reduction.
//done? Put wizard flag into player data so once used, player is always marked
//		as having "cheated".
//done? Make it so that money stolen by monsters is dropped by them when killed
...
//todo Implement AD&D cleave and whirlwind feat equivalents (just increase
//		number of attacks with level increase and allow them to spread over
//		close monsters with low hit dice etc?)

{
	...
	Modified to enable high intensity colours such that sixteen (16)
	different foreground _and_ background colours by Daniel England.

	This is the Unix version of crt.pp
...
var
	...
	UseHighIntensityColours: Boolean = False;
...
var
	CurrX, CurrY : Byte;
	OutputRedir,
	InputRedir : Boolean; { is the output/input being redirected (not a TTY) }
{$ifdef debugcrt}
	DebugFile : Text;
{$endif}
{*****************************************************************************
...
//dengland I have modified this routine to use the AIX extensions that enable
//	bright colours for foreground and background when the UseHighIntensityColours
//	is True.
}
function Attr2Ansi(attr, OAttr: longint): string;
	...
	if  not UseHighIntensityColours then
		begin
	...
	while (i < length(hstr)) do
		...
		case hstr[i] of
			'0':
				OAttr:= 7;
			'1':
//			todo need to check for 10 here
				OAttr:= OAttr or $8;
			'5':
				OAttr:= OAttr or $80;
			'3':
...
//dengland I have modified this routine to enable the use of bright colours
//		in the foreground.
}
procedure TextColor(Color: byte);
	...
	if not UseHighIntensityColours then
		...
		ttyColor((Color and $F) or (TextAttr and $F0));
	end;

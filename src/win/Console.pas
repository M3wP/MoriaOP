...
{ }
{ Port to Free Pascal by Daniel England}
...
{$IFDEF FPC}
type
//dengland FPC doesn't seem to have this definition
...
//dengland On windows, these colours can be used for the background, too.
//	Foreground colors
	DarkGray     = 8;
	LightBlue    = 9;
	LightGreen   = 10;
...
//dengland On windows, this feature is unavailable.
...
// Extra routines from FPC crt unit.
...
procedure CursorBig;
	var
	consoleCurInfo: TConsoleCursorInfo;

	begin
	consoleCurInfo.dwSize:= 100;
	consoleCurInfo.bVisible:= True;

{$IFDEF FPC}
	SetConsoleCursorInfo(TextRec(Output).Handle, @consoleCurInfo);
{$ELSE}
	SetConsoleCursorInfo(TTextRec(Output).Handle, @consoleCurInfo);
{$ENDIF}
	end;
...
function TranslateKey(const Rec: TInputRecord; State: Integer; Key: PKey;
		KeyCode: Integer): Smallint;
	begin
...
{$IFDEF INLINES}inline; {$ENDIF}
	...
	if Assigned(Key) then
		Result:= TranslateKey(Rec, Rec.Event.KeyEvent.dwControlKeyState, Key,
...
procedure ClrScr;
	...
	begin
	if  (TextWindow.Left = 0)
	and (TextWindow.Top = 0)
	and (TextWindow.Right = BufferSize.X - 1)
	and (TextWindow.Bottom = BufferSize.Y - 1) then
		begin
		StartPos.X:= 0;
		StartPos.Y:= 0;
		...
		for I     := TextWindow.Top to TextWindow.Bottom do
			...
			StartPos.Y:= I;
			FillConsoleOutputCharacterA(StdOut, ' ', Len, StartPos, NumWritten);
			FillConsoleOutputAttribute(StdOut, TextAttr, Len, StartPos,
				NumWritten);
			end;
		end;
	GotoXY(1, 1);
...
procedure DelLine;
	begin
	Scroll(TextWindow.Left, GetCursorY, TextWindow.Right,
		TextWindow.Bottom, -1);
	end;
...
{$IFDEF INLINES}inline; {$ENDIF}
	begin
	Result:= (X1 < X2) and (Y1 < Y2) and (X1 >= 0) and (X2 < BufferSize.X) and
...
{$IFDEF FPC}
function NewTextOut(var T: TextRec): Integer;
{$ELSE}
...
//	Line only contains one line of windowed text.
	WritePtr      := PAnsiChar(Line);
{$IFDEF FPC}
...
{$ENDIF}
	...
	while T.BufPos > 0 do
		...
		while (T.BufPos > 0) and (DistanceToEdge > 0) do
			...
			case ReadPtr^ of
				#7:
//					this is what my internal speaker uses.
...
procedure Window(Left, Top, Right, Bottom: Integer);
	...
	if Validate(Left, Top, Right, Bottom) then
		...
		if  (Left > 0)
		or  (Top > 0)
		or  (Right < BufferSize.X - 1)
		or  (Bottom < BufferSize.Y - 1) then
//			Text must be contained in window
			begin
{$IFDEF FPC}
			OldInOutFunc              := TextRec(Output).InOutFunc;
			OldFlushFunc              := TextRec(Output).FlushFunc;
			TextRec(Output).InOutFunc:= @NewTextOut;
			TextRec(Output).FlushFunc:= @NewTextOut;
{$ELSE}
...
{$ENDIF}
		...
		if Assigned(OldInOutFunc) then
			begin
{$IFDEF FPC}
			TextRec(Output).InOutFunc:= OldInOutFunc;
{$ELSE}
...
procedure InitScreenMode;
	...
	Rewrite(Output);
{$IFDEF FPC}
	StdIn := TextRec(Input).Handle;
	StdOut:= TextRec(Output).Handle;
{$ELSE}
...
{$IFDEF HASERROUTPUT}
	Rewrite(ErrOutput);
	{$IFDEF FPC}
	StdErr:= TextRec(ErrOutput).Handle;
	{$ELSE}
